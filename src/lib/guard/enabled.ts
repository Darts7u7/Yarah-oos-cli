 ...
 * production — nobody's workflow changes until they opt in. `YARAH_GUARD` is
...
/** Default when YARAH_GUARD is unset. Off during rollout. */
 ...
 *   1. YARAH_GUARD env — override / kill switch
 *   2. persisted project setting (`link --guard`, stored in .yarah/project.json)
...
export function guardEnabled(
  ...
  const v = (env.YARAH_GUARD ?? '').toString().trim().toLowerCase();
